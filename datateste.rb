...
dados = Mechanize.new
pagina = dados.get("https://www.qconcursos.com/questoes-de-concursos/provas/iades-2019-crf-to-analista-de-ti")
...
pagina.search(".q-discipline-item .q-heading").each do |materia|
  jd = materia.text
  numeracao = jd.split(' ')[0].strip
  if jd.index(/questão/i)
    nome_materia = jd.sub(/questão/i, "").strip
  else
    nome_materia = jd.sub(/questões/i, "").strip
  end
